...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        /// </summary>
        public static IEnumerable<string> Task14()
        {
            var deptWithEmployeeCount = Emps
                .GroupBy(e => e.Deptno)
                .Select(g => new { Deptno = g.Key, EmployeeCount = g.Count() });

            var result = Depts
                .GroupJoin(deptWithEmployeeCount,
                    d => d.Deptno,
                    e => e.Deptno,
                    (d, e) => new { Dept = d, EmployeeCount = e.FirstOrDefault()?.EmployeeCount ?? 0 })
                .Where(x => x.EmployeeCount == 5 || x.EmployeeCount == 0)
                .OrderBy(d => d.Dept.Dname)
                .Select(x => $"{x.Dept.Dname} (Deptno: {x.Dept.Deptno}, Loc: {x.Dept.Loc})");
        ...
        public static IEnumerable<object> Task16()
        {
            var result = from e in Emps
                ...
                {
                    Emp = e,
                    Dept = new { d.Deptno, d.Dname, d.Loc }
                };
